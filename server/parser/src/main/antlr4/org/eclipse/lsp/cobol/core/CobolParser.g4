...
dataDescriptionEntryFormat1
   ...
   (GENERIC_COMMENTS NONNUMERICLITERAL)?
   dot_fs   ;
...
dataDescriptionEntryFormat3
   : LEVEL_NUMBER_88 entryName? level88DataUsageClause? dataValueClause+ dot_fs
   ;
...
level88DataUsageClause
   : (USAGE IS?)? generalIdentifier
   ;
...
divideStatement
   : DIVIDE divisor (divideIntoStatement | divideIntoGivingStatement | divideByGivingStatement) divideRemainder? onSizeErrorPhrase? notOnSizeErrorPhrase? END_DIVIDE?
   ;
...
multiplyStatement
   : MULTIPLY multiplyLhs BY (multiplyRegular | multiplyGiving) onSizeErrorPhrase? notOnSizeErrorPhrase? END_MULTIPLY?
   ;
...
relationCombinedComparison
   : relationalOperator (arithmeticExpression | multipleArithmeticExpressions)
   ;

multipleArithmeticExpressions
   : LPARENCHAR arithmeticExpression additionalArithmeticExpression+ RPARENCHAR
   ;

additionalArithmeticExpression
   : (AND | OR) arithmeticExpression
   ;
